...
{
    public partial class subirAgentes : System.Web.UI.Page
    {
                        ...
                        if (FileUploadControl.PostedFile.ContentLength < 5024000)
                        {
                            ...
                            using (StreamReader sr= new StreamReader(Server.MapPath("~/DocsUp/") + filename,System.Text.Encoding.Default,false))
                            {
                                ...
                                if (string.Equals("CLAVE", campos[0]) && string.Equals("DESCRIPCION", campos[1]) 
                                    && string.Equals("ESTATUS", campos[2]) && string.Equals("TIPO_PERSONA", campos[3])
                                    && string.Equals("RFC",campos[4]) && string.Equals("EMAIL", campos[5])
                                    && string.Equals("EMAIL_ALTERNO",campos[6]) && string.Equals("PROMOTOR", campos[7])
                                    && string.Equals("PROMOTORIA",campos[8]) && string.Equals("DESC_PROMOTORIA", campos[9]))
                                {
