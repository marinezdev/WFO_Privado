...
{
    public partial class detalleMesaRv2 : System.Web.UI.Page
    {
        ...
        protected void btnExportar_Click2(object sender, EventArgs e)
        {
            ...
            if (CalDesde.Date <= CalHasta.Date)
            {
                string Encrypt = (new Application.Operacion.UrlCifrardo()).Encrypt("In=" + CalDesde.Date + ",Fn=" + CalHasta.Date + ",Us=" + manejo_sesion.Credencial.Id + ",nu=2");
                //string script = "window.open('detalleMesaRDescargaV2.aspx?In=" + CalDesde.Date + "&Fn=" + CalHasta.Date + "&Us=" + manejo_sesion.Credencial.Id + "&nu=2','Expediente', 'width = 800, height = 400');";
                string script = "window.open('detalleMesaRDescargaV2.aspx?data=" + Encrypt + "','Expediente', 'width = 800, height = 400');";
                ScriptManager.RegisterStartupScript(this, GetType(), "ServerControlScript", script, true);
