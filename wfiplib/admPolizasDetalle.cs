...
{
    /// <summary>
    /// administración de la tabla PolizasDetalle26
    /// </summary>
    public class admPolizasDetalle
    {
        BaseDeDatos b = new BaseDeDatos();

        public DataTable SeleccionarPolizasColectividad()
        {
            ...
            try
            {
                ...
                "INSERT INTO PolizasDetalle (Poliza, Dependencia, APaterno, AMaterno, Nombres, FNacimiento, RFC, CURP, Sexo, CEntidadFederativa, CMunicipio, NivelTabular, MPercepcionOBM, Eventual , APAsegurado, AMAsegurado, NAsegurado, FNAsegurado, CURPAsegurado, SAsegurado, FAAsegurado, TAsegurado, FIColectividad,  " +
                "Trimestre, SAB2T, pab2t)  " +
                "SELECT c.Poliza, c.Dependencia, c.APaterno, c.AMaterno, c.Nombres, c.FNacimiento, c.RFC, c.CURP, c.Sexo, c.CEntidadFederativa, c.CMunicipio,  " +
                ...
                "FROM AseguradosTitulares a, Coasegurados b, PolizasDetalle26 c  " +
                "WHERE a.Curp=b.CURPTitular  " +
                "AND c.Trimestre=2  " +
                "AND c.FNAsegurado=b.FechaNacimiento  " +

                "UPDATE PolizasDetalle  " +
                "SET SAP7Q=PolizasDetalle26.SAPotenciada, PAP7Q=PolizasDetalle26.ImporteTotalPagar  " +
                "FROM PolizasDetalle " +
                "INNER JOIN PolizasDetalle26 " +
                "ON (PolizasDetalle.CURPAsegurado=PolizasDetalle26.CURPAsegurado) " +
                "WHERE PolizasDetalle26.Quincena=7 " +

                "UPDATE PolizasDetalle " +
                "SET SAP8Q=PolizasDetalle26.SAPotenciada, PAP8Q=PolizasDetalle26.ImporteTotalPagar " +
                "FROM PolizasDetalle " +
                "INNER JOIN PolizasDetalle26 " +
                "ON (PolizasDetalle.CURPAsegurado=PolizasDetalle26.CURPAsegurado) " +
                "WHERE PolizasDetalle26.Quincena=8 " +

                "UPDATE PolizasDetalle " +
                "SET SAP9Q=PolizasDetalle26.SAPotenciada, PAP9Q=PolizasDetalle26.ImporteTotalPagar  " +
                "FROM PolizasDetalle  " +
                "INNER JOIN PolizasDetalle26  " +
                "ON (PolizasDetalle.CURPAsegurado=PolizasDetalle26.CURPAsegurado)  " +
                "WHERE PolizasDetalle26.Quincena=9  " +

                "UPDATE PolizasDetalle  " +
                "SET SAP10Q=PolizasDetalle26.SAPotenciada, PAP10Q=PolizasDetalle26.ImporteTotalPagar  " +
                "FROM PolizasDetalle  " +
                "INNER JOIN PolizasDetalle26  " +
                "ON (PolizasDetalle.CURPAsegurado=PolizasDetalle26.CURPAsegurado)  " +
                "WHERE PolizasDetalle26.Quincena=10  " +

                "UPDATE PolizasDetalle  " +
                "SET SAP11Q=PolizasDetalle26.SAPotenciada, PAP11Q=PolizasDetalle26.ImporteTotalPagar  " +
                "FROM PolizasDetalle  " +
                "INNER JOIN PolizasDetalle26  " +
                "ON (PolizasDetalle.CURPAsegurado=PolizasDetalle26.CURPAsegurado)  " +
                "WHERE PolizasDetalle26.Quincena=11  " +

                "UPDATE PolizasDetalle  " +
                "SET SAP12Q=PolizasDetalle26.SAPotenciada, PAP12Q=PolizasDetalle26.ImporteTotalPagar  " +
                "FROM PolizasDetalle  " +
                "INNER JOIN PolizasDetalle26  " +
                "ON (PolizasDetalle.CURPAsegurado=PolizasDetalle26.CURPAsegurado)  " +
                "WHERE PolizasDetalle26.Quincena=12  " +
                ...
                "SELECT TOP 100 Poliza, Dependencia, '' AS [NoCertificado], APaterno, AMaterno, Nombres, FNacimiento, RFC, CURP, Sexo, CEntidadFederativa, CMunicipio, NivelTabular, " +
                ...
                "FROM PolizasDetalle " +
                "GROUP BY Poliza, Dependencia, APaterno, AMaterno, Nombres, FNacimiento, RFC, CURP, Sexo, CEntidadFederativa, CMunicipio, NivelTabular, " +
...
#pragma warning restore CS0168 // La variable 'ex' se ha declarado pero nunca se usa
        ...
        public int PolizasDetalleAgregar(
            string poliza, 
            string dependencia,
            string apaterno,
            string amaterno,
        ...
        {
            string consulta = "INSERT INTO polizasdetalle26 (Poliza, Dependencia, APaterno, AMaterno, Nombres, FNacimiento, RFC, CURP, Sexo, CEntidadFederativa, " +
        ...
        public int PolizaDetallePotenciacion(string poliza,
            string dependencia,
            string apaterno,
            string amaterno,
            string nombres,
        ...
        {
            string consulta = "SELECT DISTINCT(folioasignado) AS folio, poliza, CASE estado WHEN 1 THEN 'Subido Ana/Dep' WHEN 2 THEN 'Procesado' END AS estado, dependencia FROM PolizasDetalle26";
            b.ExecuteCommandQuery(consulta);
            return b.Select();
        }

        public DataTable SeleccionarPorId(string folio)
        {
            string consulta = "SELECT * from polizasDetalle26 WHERE folioasignado=@folio";
            b.ExecuteCommandQuery(consulta);
            b.AddParameter("@folio", folio, SqlDbType.NVarChar, 20);
            return b.Select();
        }
        ...
        {
            string consulta = "DELETE FROM polizasDetalle26 WHERE folioasignado=@folio";
            b.ExecuteCommandQuery(consulta);
        ...
        public DataTable SeleccionarCienPosicionesPagos(string folio, string periodo)
        {
            string consulta = "SELECT '10' + dbo.fn_Dependencias(Dependencia) + '      ' + '" + periodo + "075P' + " + 
        ...
        public DataTable SeleccionarCienPosicionesCancelaciones(string folio, string periodo)
        {
            string consulta = "SELECT '10' + dbo.fn_Dependencias(Dependencia) + '      ' + '" + periodo + "075C' + " +
            "dbo.ConvertirImporte(REPLACE(CAST(SUM(CONVERT(FLOAT, ISNULL(REPLACE(ImporteTotalPagar, ',', ''), 0))) AS varchar), '.', '')) + " +
            "RFC + " +
            "CASE " +
            "WHEN LEN(SUBSTRING((APaterno +' ' + AMaterno + ' ' + Nombres), 1, 29)) = 29 " +
        ...
        public string SeleccionarRetenedor(string dependencia)
        {
            string consulta = "SELECT retenendor FROM Dependencias WHERE nombre=@dependencia";
            b.ExecuteCommandQuery(consulta);
            b.AddParameter("@dependencia", dependencia, SqlDbType.NVarChar, 250);
            return b.SelectString();
        }

        public bool SeleccionarDependenciaValidar(string dependencia)
        {
            bool regresar = false;
            string consulta = "SELECT Nombre FROM Dependencias WHERE nombre=@dependencia";
            b.ExecuteCommandQuery(consulta);
            b.AddParameter("@dependencia", dependencia, SqlDbType.NVarChar, 250);
            if (b.SelectString().Length > 0)
                regresar = true;
            return regresar;
        }
        ...
        {
            string consulta = "DELETE FROM Polizasdetalle26";
            b.ExecuteCommandQuery(consulta);
    ...
    public class PolizasDetallePropiedades
    {
        ...
        public string CEntidadFederativa { get; set; }
        public string CMunicipio { get; set; }
        public string NivelTabular { get; set; }
        public string MPercepcionOrdinariaBrutaMensual { get; set; }
        public string Eventual { get; set; }
        public string APAsegurado { get; set; }
        public string AMAsegurado { get; set; }
        public string NAsegurado { get; set; }
        public string FNAsegurado { get; set; }
        public string CURPAsegurado { get; set; }
        public string SAsegurado { get; set; }
        public string FAAsegurado { get; set; }
        public string TAsegurado { get; set; }
        public string FIColectividad { get; set; }
        public string SABasica { get; set; }
        public string PBTReportar { get; set; }
        public string MAPBasica { get; set; }
        public string ITPDependencia { get; set; }
    }
}
